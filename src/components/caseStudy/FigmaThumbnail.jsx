...
const FigmaThumbnail = ({ src, caption, onClick, coverImage, aspectRatio, isFigJam }) => {
  ...
  return (
    <MediaPreviewFrame
      ...
      ariaLabel={`Open Figma prototype: ${caption || "prototype"}`}
    >
      {(defaultHoverScale) => (
        <>
          {coverImage && (
            <img
              src={coverImage}
              alt=""
              className={`absolute inset-0 w-full h-full object-contain opacity-70 group-hover:opacity-80 ${defaultHoverScale} transition-all duration-700`}
            />
          )}
